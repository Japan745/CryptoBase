...
def get_dot():
            dot = yf.download("DOT-CAD")
            dot = dot.drop(columns=['Open','High','Low','Close','Volume'])
            modelhigh = sm.tsa.statespace.SARIMAX(dot['Adj Close'],
                                                  order=(1, 1, 1),
            ...
            fig = px.line(x=One_week_values.index, y=One_week_values.values,
                              labels={'x': 'Date', 'y': 'Canadian Dollars'}, title="Polkadot (DOT) forecasting",
                              markers=True)
